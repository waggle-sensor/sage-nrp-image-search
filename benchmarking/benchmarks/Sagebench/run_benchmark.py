"""Run Sagebench benchmark: load data and evaluate queries."""
...
from imsearch_eval.adapters import (
    WeaviateAdapter,
    WeaviateQuery,
    TritonModelProvider,
)
...
from config import SagebenchConfig
from data_loader import SagebenchDataLoader


config = SagebenchConfig()


def load_data(
    data_loader: SagebenchDataLoader,
    vector_db: VectorDBAdapter,
    hf_dataset,
):
    """Load Sagebench dataset into Weaviate."""
    try:
        ...
        logging.info(
            f"Successfully loaded {config.sagebench_dataset} into Weaviate collection '{config._collection_name}'"
        )
...
def run_evaluation(evaluator: BenchmarkEvaluator, hf_dataset):
    """Run the Sagebench benchmark evaluation."""
...
def main():
    ...
    benchmark_dataset = Sagebench(dataset_name=config.sagebench_dataset)
