    ...
    if config._upload_to_s3:
        if not config._s3_bucket:
            ...
            try:
                ...
                #debug
                logging.info("S3_BUCKET repr:", repr(config._s3_bucket))
                logging.info("S3_ENDPOINT repr:", repr(config._s3_endpoint))
                endpoint = config._s3_endpoint.replace("http://", "").replace("https://", "")
                logging.info("endpoint repr:", repr(endpoint))
                logging.info("S3_PREFIX repr:", repr(config._s3_prefix))
                logging.info("IMAGE_RESULTS_FILE repr:", repr(config._image_results_file))
                logging.info("timestamp repr:", repr(timestamp))
                logging.info("s3_key_image repr:", repr(s3_key_image))
