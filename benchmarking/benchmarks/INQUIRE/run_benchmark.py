...
def load_data(data_loader: INQUIREDataLoader, vector_db: VectorDBAdapter, hf_dataset: Dataset):
    ...
    try:
        ...
        # Process and stream-insert data (keeps worker pool full; inserts as chunks complete)
...
def main():
    ...
    hf_dataset = benchmark_dataset.load_as_dataset(split="test", sample_size=config.sample_size, seed=config.seed, token=(config._hf_token or None))
