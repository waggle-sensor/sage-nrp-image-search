...
| **CLIP** | Contrastive Language-Image Pre-training. The production embedding model is `DFN5B-CLIP-ViT-H-14-378` (served via Triton). Production stores caption and image embeddings as separate Milvus fields. The learning lab may still fuse them for teaching. |
